...
class Highs(_Highs):
    ...
    def __init__(self):
        super().__init__()
        self.callbacks = [HighsCallback(cb.HighsCallbackType(_), self) for _ in
                          range(int(cb.HighsCallbackType.kCallbackMax) + 1)]
        ...
        if solver_thread is not None and interrupt_limit <= 0:
            ...
            for count in range(interrupt_limit):
                try:
                    ...
                    print(
                        f"Ctrl-C pressed {count + 1} times: Waiting for HiGHS to finish. ({interrupt_limit} times to force termination)")
                    self.cancelSolve()
    ...
    # reset the objective
    def setObjective(self, obj: Optional[Union[highs_var, highs_linear_expression]] = None,
                     sense: Optional[ObjSense] = None):
        """
        Updates the costs.
    ...
    def internal_get_value(
            ...
            index_collection: Union[
                Integral, highs_var, highs_cons, highs_linear_expression, Mapping[Any, Any], Sequence[Any], np.ndarray[
                    Any, np.dtype[Any]]
            ],
    ...
    def variableValue(
            self,
            var: Union[Integral, highs_var, highs_cons, highs_linear_expression, Mapping[Any, Any], np.ndarray[
                Any, np.dtype[Any]]],
    ):
        """
    ...
    def variableValues(
            self,
            idxs: Union[Integral, highs_var, highs_cons, highs_linear_expression, Mapping[Any, Any], np.ndarray[
                Any, np.dtype[Any]]],
    ):
        """
    ...
    def constrValue(
            self,
            con: Union[Integral, highs_var, highs_cons, highs_linear_expression, Mapping[Any, Any], np.ndarray[
                Any, np.dtype[Any]]],
    ):
        """
    ...
    def constrValues(
            self,
            cons: Union[Integral, highs_var, highs_cons, highs_linear_expression, Mapping[Any, Any], np.ndarray[
                Any, np.dtype[Any]]],
    ):
        """
...
class HighsCallbackEvent(object):
    ...
    def val(
            self,
            var_expr: Union[Integral, highs_var, highs_cons, highs_linear_expression, Mapping[Any, Any], np.ndarray[
                Any, np.dtype[Any]]],
    ):
        """
...
class HighspyArray(ndarray_object_type):
    ...
    @overload
    def __getitem__(self, key: Union[SupportsIndex, tuple[SupportsIndex, ...]]) -> highs_linear_expression:
        ...  # type: ignore
    ...
    def __getitem__(self, key: Union[
        None, slice, SupportsIndex, tuple[Union[None, slice, SupportsIndex], ...]]) -> HighspyArray:
        ...

    @overload
    def __getitem__(self, key: Any) -> HighspyArray:
        ...  # type: ignore
    ...
    def sum(  # type: ignore
        ...
        if self.highs is not None:
            if axis is not None:
                return HighspyArray(
                    np.apply_along_axis(self.highs.qsum, axis, self, initial=unused_kwargs.get("initial", None)),
                    self.highs)
            else:
...
class highs_linear_expression(object):
    ...
    def __eq__(self, other: Any) -> highs_linear_expression:  # type: ignore
        ...
        # self == other
            ...
            if len(other.idxs) > len(self.idxs) or (
                    len(other.idxs) == len(self.idxs) and other.constant is None and self.constant is not None):
    ...
    def __ge__(self, other: Union[float, int, highs_var, highs_linear_expression]):
        ...
        # other <= self
            ...
            if other.bounds is None:
                ...
                # prefer most idxs, constant, 'left'
                if len(self.idxs) > len(other.idxs) or (
                        len(self.idxs) == len(other.idxs) and self.constant is None and other.constant is not None):
                    copy.idxs = self.idxs + other.idxs
                    copy.vals = self.vals + [-v for v in other.vals]
                    copy.bounds = (
                        (other.constant or 0.0) - (self.constant or 0.0),
    ...
    def __compose_chain(
        ...
        self.__reset_chain()
        assert not isinstance(inner,
                              highs_linear_expression) or inner.bounds is None, "Bounds already set in chain comparison."
